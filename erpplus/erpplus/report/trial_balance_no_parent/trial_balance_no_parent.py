...
def prepare_data(accounts, filters, parent_children_map, company_currency):
    """Return ONLY leaf-level accounts (accounts with no children)."""

    data = []

    for d in accounts:
        # TRUE leaf account → no children in tree
        is_leaf = (d.lft + 1 == d.rgt)
    ...
    # --- Add total row (sum only leaf accounts) ---
    if data:
        total_row = {
            "account": "'" + _("Total") + "'",
            "account_name": "'" + _("Total") + "'",
            "opening_debit": sum(r["opening_debit"] for r in data),
            "opening_credit": sum(r["opening_credit"] for r in data),
            "debit": sum(r["debit"] for r in data),
            "credit": sum(r["credit"] for r in data),
            "closing_debit": sum(r["closing_debit"] for r in data),
            "closing_credit": sum(r["closing_credit"] for r in data),
            "indent": 0,
            "parent_account": None,
            "currency": company_currency,
            "has_value": True,
